document.addEventListener("DOMContentLoaded", async () => {
  ...
  const toggleDnr = document.getElementById("toggle-dnr");
  ...
  // DNR state is a bit special, often managed by the browser, but we store a preference too
  toggleDnr.checked = storage.dnrEnabled !== false;
  toggleDnr.disabled = !toggleEnable.checked; // Dependent on master switch

  // Toggle Handlers
  toggleEnable.addEventListener("change", async () => {
    const enabled = toggleEnable.checked;
    await chrome.storage.local.set({ enabled });
    toggleDnr.disabled = !enabled;

    // Notify background to update icon/state
  ...
  toggleCosmetic.addEventListener("change", async () => {
    ...
    await chrome.storage.local.set({ cosmeticEnabled });
    showStatus(
      cosmeticEnabled
  ...
  toggleDnr.addEventListener("change", async () => {
    const dnrEnabled = toggleDnr.checked;
    ...
    showStatus(
      dnrEnabled ? "Network Blocking Enabled" : "Network Blocking Disabled",
    );
  });
    ...
    listContainer.querySelectorAll("input[type=checkbox]").forEach((cb) => {
      cb.addEventListener("change", async () => {
        const { filterLists: currentLists } =
          await chrome.storage.local.get("filterLists");
        const list = currentLists.find((l) => l.id === cb.dataset.id);
        if (list) {
          list.enabled = cb.checked;
          await chrome.storage.local.set({ filterLists: currentLists });
  ...
  updateListsBtn.addEventListener("click", async () => {
    ...
    try {
      await chrome.runtime.sendMessage({ type: "rebuildEngine" });
    ...
    whitelistContainer.querySelectorAll(".remove-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        const { customRules: current } =
          await chrome.storage.local.get("customRules");
  ...
  addWhitelistBtn.addEventListener("click", async () => {
    ...
    chrome.runtime.sendMessage({ type: "whitelistSite", domain });
  });
