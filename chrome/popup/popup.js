document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const { enabled, ready, blockedCount } = await chrome.runtime
      .sendMessage({
    ...
    const statusResp = await chrome.runtime.sendMessage({ type: "getStatus" });
  ...
  toggleEl.addEventListener("change", async () => {
    const resp = await chrome.runtime.sendMessage({ type: "toggleEnabled" });
  ...
  pickerBtn.addEventListener("click", () => {
    ...
    chrome.runtime.sendMessage({ type: "activatePicker", tabId: tab.id });
